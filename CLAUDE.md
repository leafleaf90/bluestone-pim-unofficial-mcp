...
- `generate_variant_matrix`: orchestrates GROUP creation or reuse, VLA configuration, SINGLE creation for each cartesian combination, attribute values, and bulk variant assignment via `append/by-ids`. Accepts attribute names and value labels; resolves definition and value IDs internally. Max 500 combinations per call.
